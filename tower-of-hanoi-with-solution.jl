...
# ╔═╡ aab29294-a2ba-11ea-3fd6-b75dacb131bf
...
starting_stacks = [collect(all_disks), [], []]
...
# ╔═╡ 730a1bae-a326-11ea-24c1-67389a1d7541
function move_stack(source, target, disks = 1)
	
	if disks == 1
		return [(source, target)]
	else
		spare = first(setdiff([1, 2, 3], [source, target]))
		clear_source = move_stack(source, spare, disks - 1)
		move_base = move_stack(source, target, 1)
		restack_target = move_stack(spare, target, disks - 1)
		
		return vcat(clear_source, move_base, restack_target)
	end
end

# ╔═╡ 010dbdbc-a2c5-11ea-34c3-837eae17416f
function solve() ::Array{Tuple{Int, Int}}
	move_stack(1, 3, num_disks)
end
...
if check_solution(solve())
	if num_disks >= 8
		...
		md"""
		Your solution works for $(num_disks) disks.
		"""
	end
else
	md"""

...
struct Ellipse
	...
	#constructors
	Ellipse(a,b) = a >= b ? new(a,b) : error("a cannot be smaller than b")
	Ellipse(a) = new(a, 1)
end
...
Remember that if you want to add a function to the plot, you have to include it in the `approximations` dict.
"""
...
$(@bind log_max_error html"<input type=range min=-4 max=1 value=-1>")
"""
...
test_ratios = range(1,max_ratio, length=100)
